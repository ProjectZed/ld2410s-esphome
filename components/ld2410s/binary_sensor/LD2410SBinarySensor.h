...
namespace esphome
{
    namespace ld2410s
    {
        ...
        public:
            ...
            {
                this->presence_bsensor = bsensor;
            };
            void set_threshold_update_sensor(binary_sensor::BinarySensor *bsensor)
            {
                this->threshold_update_bsensor = bsensor;
            };
            void on_presence(bool presence) override
            {
                if (this->presence_bsensor != nullptr && this->presence_bsensor->state != presence)
                {
            ...
            {
                if (this->threshold_update_bsensor != nullptr && this->threshold_update_bsensor->state != running)
                {
