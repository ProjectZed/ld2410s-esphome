...
namespace esphome
{
    namespace ld2410s
    {
        ...
        public:
            ...
            {
                this->distance_sensor = sensor;
            }
            void set_threshold_update_sensor(sensor::Sensor *sensor)
            {
                this->threshold_update_sensor = sensor;
            }
            void on_distance(int distance) override
            {
                if (this->distance_sensor != nullptr && this->distance_sensor->get_state() != distance)
                {
            ...
            {
                if (this->threshold_update_sensor != nullptr && this->threshold_update_sensor->get_state() != progress)
                {
